...
class GoodsCategory(models.Model):
    ...
    #目录树类别
    category_type=models.IntegerField('类目级别',choices=CATEGORY_TYPE,help_text='类目类别')
    # 一级分类: 电器    二级分类: 微波炉、电磁炉...
    # 一级分类：二级分类 = 1:N
    # 设置models有一个指向自己的外键 当作父类目级别  related_name：反向引用 父类.sub_cat 可以查看自己的子类目级别
...
class Goods(models.Model):
    ...
    # 导入from DjangoUeditor.models import UEditorField  verbose_name=u'内容':unicode 编码转换
...
class Banner(models.Model):
    ...
    def __str__(self):
        # return self.goods.name
        # 上传的本地图片的名字 图片对象的属性ImageField：包括图片size、宽度高度、图片名称等
...
class IndexAd(models.Model):
    """
    商品广告  ：一个商品广告，可以放置不同类型的商品，因此与商品分类外键相连
    """
    # 商品广告: 分类: 1:N
