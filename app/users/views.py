...
from app.users.models import UserProfile, VerifyCode
from app.users.send_messges import YunPian
...
class CustomBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        try:
            #用户名和手机号均可登录  username=1800921110X时也可以登录
...
class SmsCodeViewset(CreateModelMixin, viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True) #判断序列化对象提交的数据是否合法

        # 添加的第一个内容: 获取手机号码并发送验证码。
        mobile = serializer.validated_data["mobile"] #获取用户提交的手机号码
        yun_pian = YunPian(APIKEY)
        # 生成验证码
        code = yun_pian.generate_code()
        sms_status = yun_pian.send_sms(code=code, mobile=mobile) #给手机号发送验证码 返回一个字典包含code与msg

        if sms_status["code"] != 0:
            return Response({
                "mobile": sms_status["msg"]
            }, status=status.HTTP_400_BAD_REQUEST) #400 客户端错误
        else:
...
#没有登录账号时输入id，只返回当前登录的用户账号信息，而不是id所对应的用户的账号信息
